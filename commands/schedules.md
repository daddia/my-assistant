...
- **Working folder** — Confirm where `schedule-health/index.yaml` will be initialised. Note if the folder is read-only.
...
- Initialise `schedule-health/index.yaml`; hand copy-paste prompts and cadences from the catalog. State caveats up front (machine awake, tool pre-approval, quota).
...
- Re-read `schedule-health/index.yaml` — confirm chosen jobs and surfaces match the plan.
...
- **Details**: jobs configured, surfaces chosen, schedule-health initialised
```

## Next Steps

- Confirm heartbeats appear in `schedule-health/index.yaml` after the first run.
