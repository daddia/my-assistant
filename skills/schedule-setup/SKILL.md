...
Turn the plugin's recurring jobs into scheduled tasks. The canonical job list lives in `config/schedule-catalog.yaml` — five jobs, stable `job_id` slugs, shared prompts across local Cowork schedules, Claude Code cloud schedules, and managed-agent cookbooks.
...
Read `config/schedule-catalog.yaml` and walk the user through surface selection **per job**. Default is always `local` — managed agents are opt-in when reliability outweighs local-first.
...
Prompts below match `config/schedule-catalog.yaml`. Anchor headings are stable `packaged_prompt_ref` targets.
