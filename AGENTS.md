...
Schedule health: scheduled runs write heartbeats to `{working-folder}/scheduled/{job_id}.yaml` (one file per job) per `config/schedules.schema.yaml` and `config/schedules.yaml`. Setup and miss detection: `docs/guide/07-always-on-reliability.md`. Maintainer fixtures: [`evals/schedule-health/`](evals/schedule-health/).
